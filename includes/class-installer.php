...
class Installer {
	...
	private static function maybe_migrate_columns(): void {
		...
		if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $index_table ) ) ) {
			$existing_cols = $wpdb->get_col( "DESCRIBE `{$index_table}`", 0 ); // phpcs:ignore
			if ( is_array( $existing_cols ) ) {
				...
				foreach ( $cols_to_check as $col_name => $alter_sql ) {
					if ( ! in_array( $col_name, $existing_cols, true ) ) {
						$wpdb->query( $alter_sql ); // phpcs:ignore
					}
				}

				$wpdb->suppress_errors( true );
				$ft = $wpdb->get_var( "SHOW INDEX FROM `{$index_table}` WHERE Index_type = 'FULLTEXT' AND Key_name = 'ft_all'" ); // phpcs:ignore
				if ( ! $ft ) {
					$wpdb->query( "ALTER TABLE `{$index_table}` ADD FULLTEXT KEY `ft_all` (`title`,`content`,`excerpt`)" ); // phpcs:ignore
				}
				$wpdb->suppress_errors( false );
			}
		}
